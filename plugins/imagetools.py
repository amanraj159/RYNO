...
async def sketch(e):
    ...
    if ultt.endswith(".tgs"):
        ...
        cmd = ["lottie_convert.py", ultt, "ryno.png"]
        file = "ryno.png"
...
async def ultd(event):
    ...
    cv2.imwrite("ryno.jpg", ultroid)
    await event.client.send_file(
        event.chat_id, "ryno.jpg", force_document=False, reply_to=event.reply_to_msg_id
    )
    await xx.delete()
    os.remove("ryno.png")
    os.remove("ryno.jpg")
...
async def ultd(event):
    ...
    if not utd:
        return
    with open("ryno.png", "wb") as f:
        f.write(requests.get(ms).content)
    img = Image.open("ryno.png").convert("RGB")
